...
//nice
        ...
        while (opModeIsActive()) {
            ...
            if (runtime.seconds() <= 20 && runtime.seconds() > 10) {
                if (shooterVelocity < lowestVelocity) {
                    lowestVelocity = shooterVelocity;
                } else if (shooterVelocity > highestVelocity) {
                    highestVelocity = shooterVelocity;
                }
            }

            // Show the elapsed game time and shooter velocity. lowest highest:
            //telemetry.addData("Status", "Run Time: " + runtime.toString());
            telemetry.addData("Shooter", "velocity: %.0f", shooterVelocity);
            telemetry.addData("\nHighestVelocity: ", "\nLowestVelocity: ", highestVelocity, lowestVelocity);
            telemetry.update();
        }
        // STOP was pressed
        ((DcMotorEx) shooter).setVelocity(0);
