package org.firstinspires.ftc.teamcode.FIX_IT_Library.FXTSensors;
 ...
 * @author gai
 */
public abstract class HidBridge {
    ...
    private static final String ACTION_USB_PERMISSION =
            "com.awesome.is.alec.mouser.USB_PERMISSION";
    ...
    public HidBridge(Context context, UsbDevice usb) {
        _context = context;
        _usbDevice = usb;
    ...
    public boolean openDevice() {
        ...
        while (deviceIterator.hasNext()) {
            ...
            if (device.getVendorId() == _vendorId && device.getProductId() == _productId) {
                _usbDevice = device;
        ...
        if (_usbDevice == null) {
            Log("Cannot find the device. Did you forget to plug it?");
    ...
    public void unregisterReceiver() {
        try {
            if (registered) _context.unregisterReceiver(mUsbReceiver);
        } catch (Exception e) {
            Log("not registered");
        }
    }
